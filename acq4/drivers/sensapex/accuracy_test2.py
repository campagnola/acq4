...
for i in range(100):
    ...
    dev.goto_pos(target, speed=1000, simultaneous=True) # was: speed=1000, linear=True) 
    while dev.is_busy() and pg.ptime.time() - waitstart < 20.0:
        update()
    waitstart = pg.ptime.time()
